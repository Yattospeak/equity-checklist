...
# --------------------------------------------------------------------------
# 三档评级：优 / 中 / 低。
# 保留旧档位 key 仅为兼容历史生成的 scores.json，新报告只输出三档。
DECISION_STYLE = {
    "优": ("#00a86b", "#0d8f5f", "优"),
    "中": ("#c99700", "#a87e00", "中"),
    "低": ("#d9534f", "#c0392b", "低"),
    # 兼容旧数据
    "强烈买入": ("#00a86b", "#0d8f5f", "优"),
    "买入": ("#2e9e5b", "#248a4d", "优"),
    "买入谨慎": ("#c99700", "#a87e00", "中"),
    "谨慎": ("#e08a00", "#c07800", "中"),
    "中性": ("#8a8f98", "#6f747c", "中"),
    "回避": ("#d9534f", "#c0392b", "低"),
    "拒绝": ("#c9302c", "#a02622", "低"),
...
def decision_of(total: float) -> str:
    ...
    注意：此函数看不到「维度是否触发硬性闸门」，因此主流程应以
    auto_score.decision_text() 的结果为准，这里只在缺少 scores 时兜底。
    """
    if total >= 3.8:
        return "优"
    if total >= 3.0:
...
FOOT_TPL = """
<div class="foot">
  <b>免责声明</b>：本报告由「检查表评估模型 v2.3」自动生成，基于公开财报与公告数据的
